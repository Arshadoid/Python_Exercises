...
class HeightTracker:
    def __init__(self, win, point):
        ...
        self.point=point
        self.pos=point.getX()
        Text(Point(self.pos-5, point.getY()),"Height: ").draw(self.win)
        
        self.heightText=Text(Point(self.pos+13,point.getY()),"")
        self.heightText.draw(win)
        
    def update(self,height):
        self.heightText.undraw()
        self.heightText=Text(Point(self.pos+13,self.point.getY()),str(round(height,2)))
        self.heightText.draw(self.win)
    def reset(self):
        self.heightText.undraw()
...
def main ():
    ...
    while True:
        ...
        # Initial value to find max achieved height
        maxHeight=0
        # create a shot and track until it hits ground or leaves window
        angle, vel, height = inputwin.getValues()
        
        heightTracker=HeightTracker(win, Point(10,150))
        heightTracker.update(height)
        shot = ShotTracker(win, angle, vel, height)
        while 0 <= shot.getY() and -10 < shot.getX() <= 210:
            heightTracker.update(shot.getY())
            if maxHeight<shot.getY():
                maxHeight=shot.getY()
            
            shot.update(1/50)
            update(50)
        print("Maximum achieved height is", maxHeight)
        heightTracker.reset()
    win.close()
